...
| `agentcore_policy`       | `false`                 | `lib/common/feature-flags.ts:38` | Reserved (no resource wired yet)                                                                                                                                                                                                                                      |
| `agentcore_evaluations`  | `false`                 | `lib/common/feature-flags.ts:39` | Reserved (no resource wired yet)                                                                                                                                                                                                                                      |
| `server_side_tools`      | `true`                  | `lib/common/feature-flags.ts:40` | Flag consumed by orchestrator runtime (in-container) — no CDK resource gated                                                                                                                                                                                          |
| `durable_functions`      | `false`                 | `lib/common/feature-flags.ts:41` | Backend → `Tool_research_orchestrator` Lambda + its log group (OFF in dev)                                                                                                                                                                                            |
| `guardrails`             | `true`                  | `lib/common/feature-flags.ts:42` | Backend → `Guardrail` (`CfnGuardrail`) + guardrail SSM params                                                                                                                                                                                                         |
| `browser`                | `true`                  | `lib/common/feature-flags.ts:43` | Orchestrator runtime imports `BROWSER_TOOLS` (AgentCore Browser microVM is invoked at runtime via `bedrock-agentcore` APIs granted on `AgentCoreRole`). No standalone CDK resource — the flag toggles in-container behavior.                                          |
