...
"""Find some apartments and calculate the distance to Caltrain station.
...
from utils.utils import DataInterface, Column, save_final_output, log_relationship, create_temp_log_dirs # type: ignore
...
def add_biking_distance(
    ...
    with NovaAct(
        ...
        log_relationship(logs_directory, parent_session_id, current_nova_id)
        try:
            nova.act(
...
def main(
    ...
    # Create table output using DataInterface
    output = DataInterface.create_table(
        source=["https://zumper.com/", "https://maps.google.com/"],
        cols=[
            Column("address", "Address"),
            Column("price", "Price"),
            Column("beds", "Beds"),
            Column("baths", "Baths"),
            Column("biking_time_hours", "Hours"),
            Column("biking_time_minutes", "Minutes"),
            Column("biking_distance_miles", "Distance (miles)")
        ],
        rows=closest_apartment_data.to_dict('records'),
        title="Apartments with Biking Distance to Caltrain"
    )
    
    save_final_output(output.to_dict(), logs_directory)
...
if __name__ == "__main__":
    ...
    main(caltrain_city="Redwood City", bedrooms=2, baths=1, headless=True, min_apartments_to_find=5, logs_directory=logs, video=True)
