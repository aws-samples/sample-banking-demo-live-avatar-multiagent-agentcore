...
const propertyFilterQuerySchema: z.ZodType<PropertyFilterQuerySchemaType> =
    propertyFilterQueryBaseSchema.extend({
        tokenGroups: z.lazy(() =>
            z.array(z.union([propertyFilterTokenSchema, propertyFilterQuerySchema])).optional()
        ),
    });

export const parsePropertyFilterQuery = (
    stringifiedPropertyFilter: string
): PropertyFilterQuery => {
    ...
    try {
        ...
        console.error("Error parsing:", error instanceof Error ? error.message : "Unknown error");
        return defaultQuery;
    }
};
