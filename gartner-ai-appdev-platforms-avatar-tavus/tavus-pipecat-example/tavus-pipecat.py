...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments, pipeline_mode: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Kick off the conversation: wait for the avatar to initialize, then
        # queue an LLMRunFrame. This pushes the context to the LLM — critically,
        # for Nova Sonic this is what lets it "finish connecting" and start
        # processing user audio. Guarded so it runs only once per session.
        conversation_started = {"done": False}

        async def start_conversation(label: str):
            import asyncio
            if conversation_started["done"]:
                return
            conversation_started["done"] = True
            logger.info(f"{label}: waiting for avatar to initialize...")
        ...
        # Daily transport (cloud deployment) fires on_first_participant_joined.
        @transport.event_handler("on_first_participant_joined")
        async def on_first_participant_joined(transport, participant):
            await start_conversation("First participant joined")

        # SmallWebRTC transport (local dev) does NOT fire on_first_participant_joined;
        # it fires on_client_connected. Bind the same startup so the bot greets and
        # begins listening locally too.
