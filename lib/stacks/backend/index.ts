...
export class Backend extends Stack {
    ...
    constructor(scope: Construct, id: string, props: BackendProps) {
        ...
        // ─── CloudWatch Transaction Search (custom resource, feature-gated) ──
        // Makes X-Ray deliver trace segments to CloudWatch Logs, which is what
        // creates the shared `aws/spans` log group the batch evaluation reads.
        // There is no CloudFormation resource for it and it is otherwise a manual
        // console step, so a fresh account used to fail the evaluation step with
        // "Log group 'aws/spans' not found in your account".
        //
        // Account-level shared configuration: the provisioner reads before
        // writing, never lowers an indexing percentage another workload raised,
        // and does NOT revert on delete. Best-effort — a deployer who cannot
        // change X-Ray settings still gets a successful deploy.
        if (features.auto_enable_transaction_search) {
            const txSearchFn = new LambdaFunction(this, "TransactionSearchProvisioner", {
                functionName: `${stackName}-transaction-search-provisioner`,
                runtime: LambdaRuntime.PYTHON_3_13,
                architecture: Architecture.ARM_64,
                handler: "index.on_event",
                timeout: Duration.minutes(5),
                memorySize: 256,
                // No third-party deps: the runtime-bundled boto3 already covers
                // logs/xray/sts, so this needs no bundling step.
                code: Code.fromAsset(
                    path.join(repoRoot, "lib", "lambdas", "transaction-search-provisioner")
                ),
                environment: { SPANS_LOG_GROUP: "aws/spans", INDEXING_PERCENTAGE: "1" },
            });
            // None of these are resource-scopeable: the X-Ray trace-segment
            // destination and the Default indexing rule are account singletons,
            // and PutResourcePolicy targets the account's Logs resource policies.
            txSearchFn.addToRolePolicy(
                new PolicyStatement({
                    effect: Effect.ALLOW,
                    actions: [
            ...
            NagSuppressions.addResourceSuppressions(
                ...
                [
                    {
                        id: "AwsSolutions-IAM5",
                        reason: "Transaction Search is account-level configuration: the X-Ray trace-segment destination and Default indexing rule are singletons and Logs PutResourcePolicy is account-scoped, so none of these actions are resource-scopeable.",
                    },
                ],
                true
            );

            const txSearchResource = new CustomResource(this, "TransactionSearchResource", {
                serviceToken: new Provider(this, "TransactionSearchProvider", {
                    onEventHandler: txSearchFn,
                }).serviceToken,
                properties: {
            ...
            new CfnOutput(this, "TransactionSearchStatus_Output", {
                value: txSearchResource.getAttString("Destination"),
                description:
                    "CloudWatch Transaction Search trace-segment destination state; settles from PENDING to ACTIVE asynchronously",
            });
        }
