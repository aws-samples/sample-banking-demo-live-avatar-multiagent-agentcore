...
class TestResolvePipelineHappyPath:
    ...
    def test_explicit_strategy_research_metadata(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", {"pipeline": "strategy_research"}) == "strategy_research"

    def test_explicit_market_research_metadata(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", {"pipeline": "market_research"}) == "market_research"

    def test_explicit_services_metadata(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("menus/foo.pdf", {"pipeline": "services"}) == "services"
    ...
    def test_legacy_research_alias_maps_to_bistro(self, load_tool):
        ...
        # taxonomy was split. The alias keeps them on strategy_research.
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", {"pipeline": "research"}) == "strategy_research"

    def test_reports_prefix_fallback(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", {}) == "strategy_research"

    def test_menus_prefix_fallback(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("menus/foo.pdf", {}) == "services"

    def test_none_metadata_treated_as_empty(self, load_tool):
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", None) == "strategy_research"
    ...
    def test_bogus_metadata_falls_back_to_known_prefix(self, load_tool):
        ...
        mod = load_tool("kb_ingest")
        assert mod._resolve_pipeline("reports/foo.pdf", {"pipeline": "bogus"}) == "strategy_research"
...
class TestRequireUserId:
    ...
    def test_missing_user_id_raises(self, load_tool):
        ...
        with pytest.raises(ValueError):
            mod._require_user_id({"pipeline": "strategy_research"})

    def test_empty_user_id_raises(self, load_tool):
        mod = load_tool("kb_ingest")
        with pytest.raises(ValueError):
            mod._require_user_id({"user_id": "", "pipeline": "strategy_research"})
