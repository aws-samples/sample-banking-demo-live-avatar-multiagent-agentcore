...
- **API Gateway needs an account-level CloudWatch role**: `disableCloudWatchRole` is true in `cdk.json`, so the `RestApi` constructs do not create one. The Auth stack creates the single `CfnAccount` + role instead (both Backend and TavusAvatar depend on Auth). Without it, any stage with `loggingLevel` fails with "CloudWatch Logs role ARN must be set in account settings to enable logging" — which only shows up in an account that has never had it set.
- **AgentCore Memory blocks rollback while CREATING**: `DeleteMemory` fails with "Memory is in transitional state CREATING", so a Backend failure during memory creation leaves the stack in `ROLLBACK_FAILED`. Wait for the memory to finish, then delete the stack and redeploy. Consider `--no-rollback` on first deploys.
